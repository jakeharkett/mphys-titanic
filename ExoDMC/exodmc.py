...
class exodmc(object):
	...
	def DImode(self, xlim, ylim, lxunit='au', lyunit='Mjup', verbose=True, plot=True, savefig=True):
		...
		for ll in range(ns):
			...
			if plot is True:
				...
				contours = plt.contour(self.sma, self.M2, map.T*100, levels, cmap='bone', zorder=10, linewidths=1)
				...
				plt.title(self.ID[ll]+' RDI Sensitivity Map')
				if savefig is True: plt.savefig(self.ID[ll]+'_detprob_RDI.png', dpi=300)
